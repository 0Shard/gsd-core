 ...
 *
 * Kiro added as option 13 (alphabetical slot between kilo and opencode) —
 * opencode/pi/qwen/trae/windsurf/zcode each shift up one slot (14–19), and the
 * "All" shortcut moves from 19 to 20.
 */
...
describe('multi-runtime selection parsing', () => {
  ...
  test('single choice for kiro', () => {
    assert.deepStrictEqual(parseRuntimeInput('13'), ['kiro']);
  });
...
describe('install.js exports multi-select runtime metadata', () => {
  ...
  const expectedRuntimes = [
    ...
    'copilot', 'cursor', 'hermes', 'kimi', 'kimi-code', 'kilo', 'kiro', 'opencode', 'pi',
  ...
  test('--all flag includes Kiro exactly once', () => {
    ...
    assert.ok(selected.includes('kiro'), '--all includes kiro');
  ...
  test('prompt lists Kiro (13), pi (15), ZCode (19), and All (20)', () => {
    ...
    assert.ok(/\b13\)\s*Kiro\b/.test(prompt),
      'prompt lists Kiro as option 13');
    assert.ok(/\b15\)\s*pi\b/.test(prompt),
      'prompt lists pi as option 15');
    assert.ok(/\b16\)\s*Qwen Code\b/.test(prompt),
      'prompt lists Qwen Code as option 16');
    assert.ok(/\b17\)\s*Trae\b/.test(prompt),
      'prompt lists Trae as option 17');
    assert.ok(/\b19\)\s*ZCode\b/.test(prompt),
      'prompt lists ZCode as option 19');
